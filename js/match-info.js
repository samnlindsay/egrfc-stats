...
function metricRowHtml(label, homeValue, awayValue, formatter = 'integer', higherIsBetter = true) {
    const homeNum = numberIfFinite(homeValue);
    const awayNum = numberIfFinite(awayValue);
    if (homeNum === null && awayNum === null) return '';

    let homeLead = false;
    let awayLead = false;
    if (homeNum !== null && awayNum !== null && homeNum !== awayNum) {
        if (higherIsBetter) {
            homeLead = homeNum > awayNum;
            awayLead = awayNum > homeNum;
        } else {
            homeLead = homeNum < awayNum;
            awayLead = awayNum < homeNum;
        }
...
function renderVideoAnalysisSection(row) {
    ...
    const pair = setPieceByGameId.get(gameId);
    if (!pair) return '';

    const isEgHome = String(row?.home_away || '').trim().toUpperCase() !== 'A';
    const homeStats = isEgHome ? pair.egrfc : pair.opposition;
    const awayStats = isEgHome ? pair.opposition : pair.egrfc;
...
function renderTable() {
    ...
    tbody.innerHTML = rowsHtml || '<tr><td colspan="8" class="text-muted">No matches found for these filters.</td></tr>';
...
async function loadPage() {
    ...
    try {
        ...
        setPieceByGameId = new Map();
        (Array.isArray(setPiece) ? setPiece : []).forEach(row => {
            const gameId = String(row?.game_id || '').trim();
            if (!gameId) return;
            const team = String(row?.team || '').trim();
            const lineoutsWon = numberIfFinite(row?.lineouts_won);
            const lineoutsTotal = numberIfFinite(row?.lineouts_total);
            const scrumsWon = numberIfFinite(row?.scrums_won);
            const scrumsTotal = numberIfFinite(row?.scrums_total);
