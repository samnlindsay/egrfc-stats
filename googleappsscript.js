    ...
    function onEdit(e) {
      ...
      if (
        ...
        const scheduleSheet = getSheet(SCHEDULE_SHEET_NAME);
        generateWeekLabels(scheduleSheet);
        dataCache[SCHEDULE_SHEET_NAME] =
          scheduleSheet.getLastRow() > 0 ? scheduleSheet.getDataRange().getValues() : [];
    ...
    function onAvailabilityFormSubmit(e) {
      ...
      // Keep schedule labels in sync before mapping form week selections.
      // This avoids stale/missing Week Label values after seasonal sheet resets.
      if (sheets.schedule) {
        generateWeekLabels(sheets.schedule);
        sheetData.schedule =
          sheets.schedule.getLastRow() > 0
            ? sheets.schedule.getDataRange().getValues()
            : [];
      }
      ...
      // Rebuild headers/column structure from schedule so Availability stays aligned
      // when schedule rows/fixtures/sessions are changed for a new season.
      buildAvailability(freshAvailabilityData, sheetData.schedule, sheets.availability);
    ...
    function updatePositions(playersData, positionsSh) {
      ...
      positionsSh.getRange(34, 2, 15, 5).clearContent().setBackground("#b1b1b1"); // Backs data B–F, rows 34–48 (preserves headers at 33)
      ...
      if (attendanceHistoryData && attendanceHistoryData.length > 1) {
        ...
        attendanceHistoryData.slice(1).forEach((row) => {
          ...
          if (name && weekNumber && session) {
            ...
            // Skip historical attendance rows that do not map to the current schedule.
            // This prevents old seasons from being reintroduced into AvailabilityLatest.
            if (!weekSchedule) {
              return;
            }
    ...
    function buildAvailability(
      ...
      // --- Step 2: Process availability data
      // Keep rebuilding headers from Schedule even when there are no availability rows yet.
      const hasAvailabilityRows = availabilityLatestData.length > 1;
      const players = {};
    ...
    function createAttendanceChart(availabilitySheet) {
      ...
      // Not enough columns to build chart data safely.
      if (lastCol < 3) {
        availabilitySheet.getCharts().forEach((chart) => {
          availabilitySheet.removeChart(chart);
        });
        Logger.log("Skipping training attendance chart: insufficient columns.");
        return;
      }
      ...
      // No historical/current training columns to chart yet.
      if (chartData.length <= 1) {
        availabilitySheet.getCharts().forEach((chart) => {
          availabilitySheet.removeChart(chart);
        });
        Logger.log("Skipping training attendance chart: no chartable session data.");
        return;
      }
