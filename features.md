...
- Data filters/selectors:
  - Bespoke to each page and data being displayed (e.g. a league table can only apply to a single season, so no "All" option, and it cannot be filtered by player position, so do not include that option)
  - Unobtrusive (can be hidden when not needed)
- Intuitive URLs to specific pages
- Responsive design for mobile and desktop viewing
- Consistent styling and branding across the site

# Backend features

- All data collated in a local database, with a clear schema and efficient queries to support the frontend features
- Web scraper to extract league tables from englandrugby.com, with error handling and logging
- Data extraction from google sheets for game data and player data, with error handling and logging
- Updates to the database should be easy to perform, and ideally automated to some extent (e.g. a script that can be run to update the database with new data from google sheets and englandrugby.com)
- Updates to the front end should automatically reflect changes in the database without requiring manual updates to the code (e.g. if a new season is added to the database, it should automatically be available in the league tables and season results sections)